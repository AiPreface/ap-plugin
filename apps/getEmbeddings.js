...
export class GetEmbeddings extends plugin {
  ...
  async getEmbeddings (e) {
    ...
    if (pt_list.length == 0) {
      return e.reply(
        `当前接口还没有添加${keyword ? `包含关键词【${keyword}】的` : ''}pt文件哦`
      )
    }

    // 计算预设页数（99条每页）
    const page_count = Math.ceil(pt_list.length / 99)
    if (page > page_count) {
      return e.reply(
        `当前接口中${keyword ? `包含关键词【${keyword}】` : ''}的pt共${page_count}页哦`
      )
    }
    ...
    // 首条说明信息
    let first_message = [
      `${keyword ? `包含关键词【${keyword}】的` : ''}pt列表，共${pt_list.length}条`
    ]
    if (page_count > 1) {
      first_message = [
        `${keyword ? `包含关键词【${keyword}】的` : ''}pt列表，第${page}/${page_count}页，共${pt_list.length}条。您可发送\n#pt列表${keyword}第1页\n#pt列表${keyword}第2页\n……\n来查看对应页\n\n※pt使用方式：直接作为tag使用即可。若pt包含中文，请连同【中括号】一同复制，以避免被翻译`
      ]
    }
    data_msg.push({
