...
export class Cartoonization extends plugin {
  ...
  async WhiteBoxCartoonization (e) {
    if (!API) {
      return await e.reply(
        '请先配置图片动漫化所需API，配置教程：https://www.wolai.com/ePdgFyjmMuUR9hfd4G2XLK'
      )
    }
    if (FiguretypeUser[e.user_id]) {
      e.reply(
        '当前有任务在列表中排队，请不要重复发送，动漫化完成后会自动发送结果，如果长时间没有结果，请等待1分钟再试'
      )
      return true
    }
    e = await parseImg(e)
    if (this.e.img) {
      ...
      await fetch(API + '/', {
        ...
        .then(function (response) {
          ...
          const buffer = Buffer.from(response.data[0], 'base64')
  ...
  async getImage (e) {
    ...
    const result = await this.WhiteBoxCartoonization(e)
    if (result) {
      return true
    }
  }
