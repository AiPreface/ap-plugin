...
export class appreciate extends plugin {
  ...
  async interpretation (e) {
    ...
    if (res.status === 200) {
      if (res.data.info === '') {
        e.reply(
          '该图片无解析信息，请确保图片为Stable Diffusion的输出图片，并发送的是原图'
        )
        return false
      } else {
        const data_msg = []
        ...
        if (e.isGroup) {
          ...
          send_res = await e.reply(await e.friend.makeForwardMsg(data_msg))
        }
        if (!send_res) {
    ...
    if (!setting.appreciation.useSD) {
      if (!API) {
        return await e.reply(
          '请先配置鉴赏图片所需API，配置教程：https://ap-plugin.com/Config/docs4'
        )
      }
...
async function AppreciatePictures (e) {
  ...
  if (e.img) {
    ...
    const base64 = await pic_tools.url_to_base64(e.img[0])
    const setting = await Config.getSetting()
    const model = setting.appreciation.model
    if (setting.appreciation.useSD) {
      await e.reply([
...
export async function requestAppreciate (base64) {
  ...
  try {
    let res = await fetch(API, {
      ...
      body: JSON.stringify({
        data: ['data:image/png;base64,' + base64, 0.3]
      })
    })
    res = await res.json()
    const tags = res.data[2].confidences
    let tags_str = ''
...
export async function requestAppreciateSD (base64) {
  ...
  const url = apiobj.url + '/tagger/v1/interrogate'
  const headers = {
    'Content-Type': 'application/json'
  }
