...
let FiguretypeUser = {}
let getImagetime = {}
const creatConvoluteAverage = (w) => new Array(w * w)
	.fill(1 / (w * w))
var Convolutes = {
...
export class louvreimg extends plugin {
	...
	async louvreImg(e) {
		...
		// Log.i(outputCanvas)                         //
		// return true
		var res = outputCanvas.toDataURL();
		let end = new Date()
		let time = moment(end - start)
			.format('mm:ss')
	...
	async getImage(e) {
		...
		let result = await this.louvreImg(e);
		if (result) {
			return true;
		}
	}
	async louvreHelp(e) {
		e.reply("可设置的参数有：\n精细程度：精细，一般，稍粗，超粗，极粗，浮雕（五选一）\n降噪，水印，Kiss，初回（可选）\n线迹轻重（80-126）\n调子数量（20-200）\n例：#卢浮宫精细降噪水印Kiss初回线迹轻重118调子数量108", true);
		return true;
	}
}