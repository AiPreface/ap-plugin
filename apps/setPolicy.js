...
export class setpolicy extends plugin {
  ...
  /** 设置全局cd     */
  async setcd (e) {
    const regExp = /^#ap设置全局cd(\d{1,5})$/i
    const ret = regExp.exec(e.msg)
    if (!ret) return false
    const num = Number(ret[1].trim()) || 1
  ...
  /** 设置本地检索图片张数  */
  async setlocalNum (e) {
    const regExp = /^#ap设置本地(\d{1,5})$/
    const ret = regExp.exec(e.msg)
    if (!ret) return false
    const num = Number(ret[1].trim())
    const policy = await Config.getPolicy()
    policy.localNum = num
    try {
      await Config.setPolicy(policy)
  ...
  async setisDownload (e) {
    let isOpen = true
    if (e.msg.includes('关闭')) isOpen = false
  ...
  async setapMaster (e) {
    let qq = NaN
    let isgive = true
    if (e.msg.includes('解除')) {
      isgive = false
    }

    const regExp = /^#ap(设置|解除)管理权限(\d{5,11})$/
    const ret = regExp.exec(e.msg)
    if (ret) {
      qq = Number(ret[2])
    }
    ...
    if (isgive) {
      ...
      const index = policy.apMaster.indexOf(qq)
  ...
  async setpbuser (e) {
    ...
    let qq = NaN
    let isban = true
    if (e.msg.includes('解封')) {
      isban = false
    }

    const regExp = /^#ap(封禁|解封)(\d{5,11})$/
    const ret = regExp.exec(e.msg)
    if (ret) {
      qq = Number(ret[2])
    }
    ...
    if (isban) {
      ...
      const index = policy.prohibitedUserList.indexOf(qq)
  ...
  async setgp (e) {
    ...
    const regenable = /#ap(全局)?设置(\d{5,11}|私聊)?(开启|关闭)$/
    const regJH = /#ap(全局)?设置(\d{5,11}|私聊)?审核(开启|关闭)$/
    const regisRecall = /#ap(全局)?设置(\d{5,11}|私聊)?撤回(开启|关闭)$/
    const regisBan = /#ap(全局)?设置(\d{5,11}|私聊)?封禁(开启|关闭)$/
    const reggcd = /#ap(全局)?设置(\d{5,11}|私聊)?群聊cd(\d{1,5})$/i
    const regpcd = /#ap(全局)?设置(\d{5,11}|私聊)?个人cd(\d{1,5})$/i
    const regrecallDelay = /#ap(全局)?设置(\d{5,11}|私聊)?撤回时间(\d{1,5})$/
    const regusageLimit = /#ap(全局)?设置(\d{5,11}|私聊)?次数(\d{1,5}|无限)$/

    const [enable, JH, gcd, pcd, isRecall, recallDelay, isBan, usageLimit] = [
  ...
  async masterlist (e) {
    ...
    const apMaster = []
    let i = 1
    for (const val of policy.apMaster) {
      apMaster.push(
        `${i}. ${await getuserName(e, val)}` +
          (e.isPrivate && e.isMaster ? `(${val})` : '')
      )
      i++
    }
  ...
  async banlist (e) {
    ...
    e.reply('发送中，请稍等')
    let data_msg = []
    for (const val of policy.prohibitedUserList) {
      data_msg.push({
        message: [
  ...
  async changePbWord (e) {
    if (/^#ap(添加|删除)屏蔽词$/.test(e.msg)) {
      return e.reply(
        '发送#ap添加或删除屏蔽词XXX以添加或删除屏蔽词，支持多个词，英文不需区分大小写，以逗号分隔。\n例如：#ap添加屏蔽词抬腿,掀裙子,cum\n#ap删除屏蔽词bare leg, bare foot'
      )
    }
    let isadd = true
    if (/^#ap删除屏蔽词/.test(e.msg)) {
      isadd = false
    }
    ...
    const pblist = await Config.getProhibitedWords()
    const a = []
    const b = []
    if (isadd) {
      ...
      for (const val of rawList) {
        if (!pblist.includes(val)) {
          ...
          a.push(val)
          const index = pblist.indexOf(val)
  ...
  async pbWordList (e) {
    const pblist = await Config.getProhibitedWords()
    if (pblist.length == 0) return e.reply('当前没有屏蔽词哦')
    const data_msg = []
    let text = ''
  ...
  async gp_Property (gid, key, value) {
    ...
    if (!(gid in policy.gp)) policy.gp[gid] = {}
    for (const keys in policy.gp) {
      if (gid != 'global') {
    ...
    const msg = [
      ...
      key == 'enable' || key == 'JH' || key == 'isRecall' || key == 'isBan'
        ? value
          ? '开启'
          : '关闭'
        : key == 'gcd' || key == 'pcd' || key == 'recallDelay'
          ? `${value}秒`
          : key == 'usageLimit'
            ? value
              ? `${value}张`
  ...
  integrateProperty (gp) {
    for (const each in gp) {
      if (each == 'global') continue
      for (const key in gp[each]) {
        if (gp[each][key] == gp.global[key]) delete gp[each][key]
      }
      if (JSON.stringify(gp[each]) === '{}') delete gp[each]
    }
    return gp
  }
}
