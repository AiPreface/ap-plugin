  ...
  async nat2pmpt (e) {
    if (!API) {
      return await e.reply(
        '请先配置处理所需的OpenAI Key，配置教程：https://www.wolai.com/5F1iaMYnirG8Rff3toGVEx'
      )
    }
    const lang = e.msg.replace(/^#处理/, '')
    if (!lang) return await e.reply('请输入要处理的自然语言')
          ...
          messages: [
            {
              ...
              content:
                '请为我的描述的图像生成不得少于' +
                len +
                '个的英文prompt，每个prompt用英文逗号分割，不用标注序号，用小括号将prompt与其权重包起来，比如(girls:1.3)，请不要输出任何中文和额外的解释，以下是我描述的图像：' +
                lang
            }
          ],
          temperature: 0.7
