...
export class set extends plugin {
  ...
  async addapi(e) {
    ...
    if (/hf.space/.test(api))
      return e.reply(
        "大清晰术和鉴赏的接口无法用于绘图哦，详见https://www.wolai.com/tiamcvmiaLJLePhTr4LAJE"
      );
  ...
  async selectapi(e) {
    ...
    if (num > apcfg.APIList.length || num < 1) {
      ...
      for (let val of apcfg.APIList) {
        li.push(
          `${i}：${val.remark}${i == apcfg.usingAPI ? " [当前]" : ""}${
    ...
    Config.setcfg(apcfg);
    e.reply(
      `默认接口已设置为${num}：${apcfg.APIList[num - 1].remark}${
  ...
  async set_sd_info(e) {
    ...
    Config.setcfg(apcfg);
    e.reply(
      `接口${num}：${apcfg.APIList[num - 1].remark}${
        e.isPrivate && e.isMaster ? `\n  ${apcfg.APIList[num - 1].url}` : ""
  ...
  async delapi(e) {
    ...
    let msg = [
      `成功删除接口${num}：${target.remark}${
        e.isPrivate && e.isMaster ? `\n  ${target.url}` : ""
      }`,
      apcfg.APIList.length == 0
        ? "\n当前接口列表为空"
        : ischange
        ? `\n默认接口已更改为${apcfg.usingAPI}：${
            apcfg.APIList[apcfg.usingAPI - 1].remark
          }${
            e.isPrivate && e.isMaster
  ...
  async setother(e) {
    ...
    let openai_key_reg = /^#ap设置OpenAI密钥 ?(sk-.+)$/;
  ...
  async samplerlist(e) {
    ...
    try {
      ...
      res = await res.json();
      let samplerList = [];
      for (let val of res) samplerList.push(val.name);
