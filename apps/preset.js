...
export class setpreset extends plugin {
  ...
  /** 添加预设     */
  async addPreset (e) {
    const raw_preset = await this.parsePreset(e)
    Log.i(raw_preset)
    if (!raw_preset) {
      e.reply(
    ...
    const presets = await Config.getPresets()
    for (let i = 0; i < presets.length; i++) {
      ...
      if (
        ...
        if (presets[i].keywords.includes(raw_preset.keywords[0])) {
          // 预设名也相同
      ...
      // 仅预设名相同时
        ...
        e.reply(
          `已包含名为【${raw_preset.keywords[0]}】的预设：\n${this.stringifyPreset(presets[i])}\n\n请更换预设名，或先删除同名预设再添加`
        )
        return true
      }
    }
  ...
  /* 处理消息，解析为ap标准预设格式 */
  async parsePreset (e) {
    const regExp = /^#ap添加预设([\s\S]+)=>([\s\S]+)$/
    const reg_result = regExp.exec(e.msg)
    ...
    // 解析预设的value
    const res = await Parse.parsetxt(reg_result[2], false, e)
    // Log.w(res)
    // 回填pt
    if (res.param.pt.length) {
      res.param.pt.forEach((value, index) => {
        res.param.pt[index] = `【${value}】`
      })
      res.param.tags = `${res.param.pt.join(',')},` + res.param.tags
    }
    if (res.param.npt.length) {
      res.param.npt.forEach((value, index) => {
        res.param.npt[index] = `【${value}】`
      })
    ...
    if (res.param.scale) raw_preset.param.scale = res.param.scale
    return raw_preset
  }
  ...
  async delPreset (e) {
    ...
    const presets = await Config.getPresets()
    for (let i = 0; i < presets.length; i++) {
      const index = presets[i].keywords.indexOf(kword)
      if (index > -1) {
  ...
  async presetList (e) {
    ...
    const regExp = /^#ap预设列表(.*?)(第(\d+)页)?$/
    e.msg = chNum2Num(e.msg, {
      regExp: '第([一二三四五六七八九十零百千万亿]+?)页$'
    }) // 将中文数字替换为阿拉伯数字
    ...
    if (keyword) {
      presets = presets.filter((x) => {
        return x.keywords.join('').includes(keyword)
      })
    }

    if (presets.length == 0) {
      return e.reply(
        `当前还没有${keyword ? `包含关键词【${keyword}】的` : ''}预设哦`
      )
    }

    // 计算预设页数（50条每页）
    const page_count = Math.ceil(presets.length / 50)
    if (page > page_count) {
      return e.reply(
        `${keyword ? `包含关键词【${keyword}】的` : ''}预设共${page_count}页哦`
      )
    }
    ...
    // 首条说明信息
    let first_message = [
      `${keyword ? `包含关键词【${keyword}】的` : ''}ap预设列表，共${presets.length}条`
    ]
    if (page_count > 1) {
      first_message = [
        `${keyword ? `包含关键词【${keyword}】的` : ''}ap预设列表，第${page}/${page_count}页，共${presets.length}条。您可发送\n#ap预设列表${keyword}第1页\n#ap预设列表${keyword}第二页\n……\n来查看对应页`
      ]
    }
    data_msg.push({
    ...
    // 处理每一条预设
    for (const val of selected_page) {
      data_msg.push({
  ...
  stringifyPreset (preset) {
    const param = []
    if (preset.param) {
      for (const key in preset.param) param.push(`${key}=${preset.param[key]}`)
    }
    const message = [
