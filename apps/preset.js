...
export class setpreset extends plugin {
  ...
  /** 添加预设     */
  async addPreset (e) {
    const raw_preset = await this.parsePreset(e.msg)
    Log.i(raw_preset)
    if (!raw_preset) {
      e.reply(
    ...
    for (let i = 0; i < presets.length; i++) {
      ...
      // 仅预设名相同时
        ...
        e.reply(
          `已包含名为【${
            raw_preset.keywords[0]
          }】的预设：\n${this.stringifyPreset(
            presets[i]
          )}\n\n请更换预设名，或先删除同名预设再添加`
        )
        return true
  ...
  async parsePreset (msg) {
    ...
    const reg_result = regExp.exec(msg)
    ...
    // 解析预设的value
    const res = await Parse.parsetxt(reg_result[2], false)
    // Log.w(res)
    // 回填pt
  ...
  async presetList (e) {
    ...
    // 首条说明信息
    let first_message = [
      `${keyword ? `包含关键词【${keyword}】的` : ''}ap预设列表，共${
        presets.length
      }条`
    ]
    if (page_count > 1) {
      first_message = [
        `${
          keyword ? `包含关键词【${keyword}】的` : ''
        }ap预设列表，第${page}/${page_count}页，共${
          presets.length
        }条。您可发送\n#ap预设列表${keyword}第1页\n#ap预设列表${keyword}第二页\n……\n来查看对应页`
      ]
    }
