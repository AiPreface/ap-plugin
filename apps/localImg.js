...
export class LocalImg extends plugin {
  ...
  async searchLocalImg (e) {
    ...
    const exec = /^#?ap检索(本地)?图片([\s\S]*?)(第(.*)页)?$/.exec(e.msg)
    // Log.i(exec)                            /*  */
    const key_word = exec[2]
    const page = exec[4] || 1
    ...
    fs.readdirSync(dirPath).forEach((fileName) => {
      if (key_word) {
        if (fileName.includes(key_word) && fileName.endsWith('png')) {
    ...
    const page_count = Math.ceil(fileList.length / policy.localNum)
    if (page > page_count) {
      return e.reply(
        `当前本地${key_word ? `包含关键词【${key_word}】的` : ''}图片共${page_count}页哦`
      )
    }

    // 取出指定的一页图片
    let selected_page = []
    selected_page = fileList.slice(
      (page - 1) * policy.localNum,
      page * policy.localNum
    )
    ...
    // 首条说明信息
    let first_message = [
      `${key_word ? `包含关键词【${key_word}】的` : ''}本地图片：共${fileList.length}张\n\nps:过长的tag可能不会完整显示`
    ]
    if (page_count > 1) {
      first_message = [
        `${key_word ? `包含关键词【${key_word}】的` : ''}本地图片：第${page}/${page_count}页，共${fileList.length}张。您可发送\n#ap检索本地图片${key_word}第1页\n#ap检索本地图片${key_word}第二页\n……\n来查看对应页\n\nps:过长的tag可能不会完整显示`
      ]
    }
    data_msg.push({
    ...
    for (const val of selected_page) {
      ...
      const bitMap = fs.readFileSync(picPath)
      const base64 = Buffer.from(bitMap, 'binary').toString('base64')
      const exec =
        /(\d{2})(\d{2})(\d{2})_(\d{2})(\d{2})(\d{2})_Tags=(.*)&seed=(.+)&user=(.+).png/.exec(
          val
        )
      if (exec) {
        const [YY, MM, DD, HH, mm, ss, tags, seed, qq] = [
          exec[1],
          exec[2],
          exec[3],
          exec[4],
        ...
        // Log.i(exec)
        const [tag, ntag] = tags.split('&nTags=')
        const name = await getuserName(e, qq)
  ...
  async deleteLocalImg (e) {
    const exec = /^#?ap删除(本地)?图片([\s\S]*)$/.exec(e.msg)
    // Log.i(exec)
    const key_word = exec[2]
    if (!key_word) {
      return e.reply(
        '在命令后附带关键词，以删除包含该关键词的图片；或发送#ap清空本地图片，以删除全部图片'
      )
    }
    ...
    for (const val of fileList) {
      const picPath = path.join(dirPath, val)
