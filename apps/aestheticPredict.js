...
export class score extends plugin {
  ...
  async score (e) {
    if (!API) {
      return await e.reply(
        '请先配置动漫审美预测所需API，配置教程：https://www.wolai.com/oahXrMjG1KcxeqxrKLSa7k'
      )
    }
    if (FiguretypeUser[e.user_id]) {
      e.reply(
        '当前有任务在列表中排队，请不要重复发送，取得结果后完成后会自动发送结果，如果长时间没有结果，请等待1分钟再试'
      )
      return true
    }
    e = await parseImg(e)
    if (this.e.img) {
      ...
      await fetch(API, {
        ...
        .then((r) => r.json())
        .then((r) => {
          const score = (r.data[0] * 100).toFixed(2)
          const end = new Date()
  ...
  async getImage (e) {
    ...
    const result = await this.score(e)
    if (result) {
      return true
    }
  }
