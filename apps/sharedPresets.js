...
const file_path = path.join(
  _path,
  '/plugins/ap-plugin/config/config/preset.json'
)
...
export class example extends plugin {
  ...
  async Share_preset (e) {
    const data = await Config.getPresets()
    const count = data.length
    const size = fs.statSync(file_path).size / 1024
    if (size > 10000) {
      e.reply(
        '本机预设文件过大，无法导出，最高支持10MB预设文件\n当前预设文件大小为' +
          size.toFixed(2) +
          'KB'
      )
      return true
    ...
    try {
      var res = await Upload(file_path)
    } catch (err) {
      return e.reply('上传失败，可能服务器暂时脱机，请稍后重试')
    }
    if (!res) {
      return e.reply(text)
    }
    const code = res.split('/').pop().split('.')[0]
  ...
  async Import_Presets (e) {
    ...
    // }
    try {
      var res = await Download(e.msg)
    } catch (err) {
      return e.reply('下载失败，可能服务器暂时脱机，请稍后重试')
    }
    if (!res) {
      return e.reply(text)
    }
    ...
    for (const val in res_data) {
      if (!res_data[val].keywords) {
        e.reply(
          '远程预设文件第' + (val + 1) + '个数据keywords参数错误，无法导入'
        )
        return true
    ...
    for (const val in res_data) {
      for (const val2 in data) {
        ...
        if (
          res_data[val].pt == data[val2].pt &&
          JSON.stringify(res_data[val].param) ==
            JSON.stringify(data[val2].param) &&
          res_data[val].tags == data[val2].tags &&
          res_data[val].ntags == data[val2].ntags
        ) {
