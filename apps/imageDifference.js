...
export class Difference extends plugin {
  ...
  async Difference (e) {
    ...
    const imgInfo = await Pictools.getPicInfo(this.e.img[0])
    if (!imgInfo.ok) {
      e.reply('图片坏啦！都怪你！', true)
      return true
    }
    const base64 = imgInfo.base64
    const width = Math.round(imgInfo.width / 64) * 64
    const height = Math.round(imgInfo.height / 64) * 64
    ...
    const strength = (0.5 / (num - 1)).toFixed(2)
    const msg = []
    ...
    for (let i = 0; i < num; i++) {
      ...
      if (i == 0) {
        ...
        const min = Math.floor((time * num) / 1000 / 60)
        const sec = Math.floor(((time * num) / 1000) % 60)
