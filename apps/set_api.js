...
export class set extends plugin {
	...
	async writecfg(ret, type) {
		...
		if ((type == 'appreciate' || type == 'ai_detect' || type == 'remove_bg') && value.endsWith('/')) value = value.replace(/\/$/, "")
			.trim()
		if (type == "cartoonization") value = value.replace('/+/', '/')
			.replace(/\/$/, "")
	...
	async setTranslateToken(e) {
		...
		if (apcfg[company].id && apcfg[company].key) {
			if (await this.testTranslate(company_, apcfg[company])) {
				setTimeout(() => {
					e.reply(`${company_}翻译配置成功，将优先使用该翻译接口。请重启Bot以应用配置`)
				}, 500)
			} else {
				setTimeout(() => {
