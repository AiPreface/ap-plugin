...
export class AiDetect extends plugin {
  ...
  async anime_ai_detect (e) {
    if (!API) {
      return await e.reply(
        '请先配置鉴定图片所需API，配置教程：https://www.wolai.com/3koPDP8wEne97evw1bjDiL'
      )
    }
    if (FiguretypeUser[e.user_id]) {
      e.reply(
        '当前有任务在列表中排队，请不要重复发送，鉴定完成后会自动发送结果，如果长时间没有结果，请等待1分钟再试'
      )
      return true
    }
    e = await parseImg(e)
    if (this.e.img) {
      ...
      await fetch(API, {
        ...
        .then((json) => {
          ...
          ai = (ai * 100).toFixed(2)
          human = (human * 100).toFixed(2)
          const end = new Date()
          const time = ((end.getTime() - start.getTime()) / 1000).toFixed(2)
          if (human > ai) {
            e.reply(
              `鉴定完成，耗时${time}秒，这张画像【不是AI制作的】，置信率概率为${human}%`,
              true
            )
          } else {
            if (ai < 96) {
              e.reply(
                `鉴定完成，耗时${time}秒，这张画像【是AI制作的】，置信率概率为${ai}%\n※：这并不意味着这张画像一定是AI制作的，因为结果置信率低于96%，数据仅供参考`,
                true
              )
            } else {
              e.reply(
  ...
  async getImage (e) {
    ...
    const result = await this.anime_ai_detect(e)
    if (result) {
      return true
    }
  }
