...
export class RemoveBackground extends plugin {
  ...
  async AnimeRemoveBackground (e) {
    if (!URL) {
      return await e.reply(
        '请先配置去背景所需API，配置教程：https://www.wolai.com/sSZM1AHnBULxyc4s4hKquF'
      )
    }
    // 将URL处理成API格式
    const API =
    ...
    if (FiguretypeUser[e.user_id]) {
      e.reply(
        '当前有任务在列表中排队，请不要重复发送，去背景完成后会自动发送结果，如果长时间没有结果，请等待1分钟再试',
        false,
        { at: true, recallMsg: 15 }
      )
      return true
    }
    e = await parseImg(e)
    ...
    if (this.e.img) {
      ...
      e.reply(`耗时${time}s，正在发送结果...`, false, {
        at: true,
        recallMsg: 5
      })
      res.data.data.data[1] = res.data.data.data[1].replace(
        /^data:image\/png;base64,/,
        ''
      )
      const buffer = Buffer.from(res.data.data.data[1], 'base64')
  ...
  async getImage (e) {
    ...
    const result = await this.AnimeRemoveBackground(e)
    if (result) {
      return true
    }
  }
