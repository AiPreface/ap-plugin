...
export class SR extends plugin {
  ...
  async Real_CUGAN (e) {
    if (!api) {
      return await e.reply(
        '请先配置大清晰术所需接口，参考文档：https://ap-plugin.com/Config/docs3'
      )
    }
    ...
    if (!e.img) {
      return await e.reply('请对图片回复，或在此命令后带上图片', false, {
        at: true
      })
    }
    ...
    const res = await pic_tools.getPicInfo(e.img[0])
    if (!res.ok) {
    ...
    else if (e.msg.match(/(三|3)重吟?唱?/) && res.height * res.width < 400000) {
      scale = 3
    } else if (
      e.msg.match(/(四|4)重吟?唱?/) &&
      res.height * res.width < 400000
    ) {
      scale = 4
    }
    ...
    // modelname
    const modelname = `up${scale}x-latest-${con}.pth`

    e.reply(
      [
        `源图${res.width}x${res.height}，使用${scale}重唱${con}分支，大清晰术！`
      ],
      false,
      {
        recallMsg: 30,
    ...
    try {
      result = await axios.post(
        api,
        JSON.stringify({
          data: ['data:image/jpeg;base64,' + res.base64, modelname, 2]
        }),
        {
          timeout: 3 * 60 * 1000,
    ...
    try {
      e.reply(
        [
          segment.image(
            `base64://${result.data.data[0].replace(/data:image\/png;|base64,/g, '')}`
          ),
  ...
  async SDSR (e) {
    ...
    let scale = e.msg.match(
      /^#?清晰术([1-8](\.[0-9]{1,2})?)倍(强度([0-1](\.[0-9]{1,3})?))?$/
    )
    if (!scale) scale = 2
    ...
    await e.reply(
      [
        `源图${res.width}x${res.height}，使用${setting.realesrgan.model1}算法与${setting.realesrgan.model2}算法，${scale}倍放大，强度${strength}，大清晰术！`
      ],
      false,
      { recallMsg: 30, at: true }
    )
    const config = await Config.getcfg()
    const apiobj = config.APIList[config.usingAPI - 1]
    const url = apiobj.url + '/sdapi/v1/extra-single-image'
    const headers = {
    ...
    try {
      ...
      await e.reply(
        segment.image(
          `base64://${response.data.image.replace(/data:image\/png;|base64,/g, '')}`
        ),
        true
      )
    } catch (error) {
