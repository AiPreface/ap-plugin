 ...
 * @LastEditTime: 2023-02-09 23:10:38
...
export class checkDependencies extends plugin {
  ...
  async checkDependencies(e) {
    ...
    try {
      ...
      let { error, stdout, stderr } = await this.execSync(`npm install -g pnpm`);
      if (error) {
    ...
    for (let key in packageList){
      try {
        let installedList = execSync(`cd ./plugins/ap-plugin && pnpm list ${key}`).toString();
  ...
  async installDependencies(e) {
    ...
    if (installList.length > 0) {
      ...
      e.reply(`所有依赖安装完成，机器人即将重启，尽情享受AP-Plugin的所有功能吧！`);
      common.sleep(3000);
