...
export class Anime_me extends plugin {
  ...
  async ercy (e) {
    // 获取本群策略
    const gpolicy = await Parse.parsecfg(e)
    // 判断功能是否开启
    ...
    // 判断cd
    const cdCheck = await CD.checkCD(e, gpolicy)
    if (cdCheck) return await e.reply(cdCheck, true, { recallMsg: 15 })
    ...
    // 优先取redis中的缓存数据
    let dsc = await JSON.parse(
      await redis.get(`Yz:AiPainting:ercydata:${this.qq}`)
    )
    if (!dsc) {
      ...
      redis.set(`Yz:AiPainting:ercydata:${this.qq}`, JSON.stringify(dsc), {
        EX: exTime
      })
    }
    // 用户名
    const name = await getuserName(e, this.qq)

    // 构造绘图参数
    const paramdata = await this.construct_param(dsc)
    const logMessage =
      e.msg.startsWith('%') || e.msg.startsWith('/')
    ...
    if (!setting.anime_me_card) {
      return await e.reply(
        [
          this.e.msg.startsWith('/') ? '' : `${dsc.ch.replace('_name_', name)}`,
      ...
      const img = await puppeteer.screenshot('animeMe', data)
      e.reply(img, true)
    }
  }
  ...
  async construct_param (dsc) {
    ...
    if (this.e.msg.startsWith('#')) {
      const res = await Pictools.getPicInfo(
        `https://q1.qlogo.cn/g?b=qq&s=0&nk=${this.qq}`
      )
      if (res.ok) base64 = res.base64
    ...
    const paramdata = {
      param: {
        ...
        tags: txdsc
          ? txdsc + ',' + (this.e.msg.startsWith('/') ? '' : dsc.en)
          : dsc.en + '(upper body: 1.8), (solo: 1.5)',
        ntags: '',
  ...
  async refreshErcy (e) {
    ...
    const all_list = await redis.keys('Yz:AiPainting:ercydata:*')
    // Log.i(all_list)
