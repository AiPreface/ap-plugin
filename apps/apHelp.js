...
export class apHelp extends plugin {
        ...
        if (server && key) {
            await axios.get(server, {
                ...
                if (err.response == undefined) {
                    ...
                    e.reply(`设置失败，密钥错误，请在服务端打开server_config.ini查看或更改密钥`, true);
                } else {
                    e.reply(`设置失败，这个接口似乎不是AP助手的接口`, true);
                }
            });
        } else {
            e.reply(`设置失败，格式错误，正确格式为：#ap设置ap助手<AP助手监听地址>密钥<密钥>`, true);
        }
        return true;
    }
            ...
            if (type == 'ckpt' || type == 'vae' || type == 'emb' || type == 'lora') {
                ...
                let response = await axios.post(`${server}/download`, form,
                {
                    ...
                    if (err.response == undefined) {
                        ...
                        e.reply(`下载失败，密钥错误，请在服务端打开server_config.ini查看或更改密钥`, true);
                    } else {
                ...
                if (response) {
                    if (response.data.status == 'ok') {
                        let file_name = response.data.msg.match(/(?<=链接:.*\/).*?(?=开始下载)/g);
                        e.reply(`正在下载[${response.data.gid}]${file_name}\n发送#ap下载状态查看下载进度`, true);
                    } else {
                        e.reply(`下载失败，AP助手内部错误`, true);
                    }
                }
            } else {
                e.reply(`下载失败，格式错误，正确格式为：#ap下载<ckpt|vae|emb|lora>模型<下载地址>`, true);
            }
        }
        return true;
    }
    async downloadStatus(e) {
        ...
        let response = await axios.get(`${server}/download_status`, {
            headers: {
                Authorization: key
            }
        }, { timeout: 10000
            ...
            if (err.response == undefined) {
                ...
                e.reply(`查询失败，密钥错误，请在服务端打开server_config.ini查看或更改密钥`, true);
            } else {
    ...
    async stopDownload(e) {
        ...
        gid = gid.toString();
        if (gid) {
            let response = await axios.get(`${server}/download_stop/${gid}`, {
                headers: {
                    Authorization: key
                }
            }, { timeout: 10000
                ...
                if (err.response == undefined) {
                    ...
                    e.reply(`停止失败，密钥错误，请在服务端打开server_config.ini查看或更改密钥`, true);
                } else {
