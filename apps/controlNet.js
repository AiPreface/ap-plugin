...
const configPath =
  process.cwd() + '/plugins/ap-plugin/config/config/controlnet.yaml'
...
export class ControlNet extends plugin {
  ...
  async controlNet (e) {
    ...
    const parseData = YAML.parse(
      fs.readFileSync(
        process.cwd() + '/plugins/ap-plugin/config/config/parse.yaml',
        'utf8'
      )
    )
    if (!config[e.user_id]) {
    ...
    const paramData = parseData[e.user_id] || parseData.default
    ...
    let header = {}
    if (
      apcfg.APIList[apcfg.usingAPI - 1].account_id &&
      apcfg.APIList[apcfg.usingAPI - 1].account_password
    ) {
      header = {
        Authorization:
          'Basic ' +
          Buffer.from(
            `${apcfg.APIList[apcfg.usingAPI - 1].account_id}:${apcfg.APIList[apcfg.usingAPI - 1].account_password}`
          ).toString('base64')
      }
    }
    try {
      ...
      sendMsg(e, [
        ...
        segment.image(`base64://${response.data.images[1]}`)
      ])
    } catch (error) {
      e.reply('生成失败', true)
      Log.e(error)
  ...
  async controlNetPreprocess (e) {
    ...
    try {
      ...
      e.reply([
        `● 图片生成成功\n◎ 使用的预处理器：${config[e.user_id].module}\n◎ 蒙版图片：`,
        segment.image(`base64://${res.data.images[0]}`)
      ])
      return true
    } catch (error) {
      e.reply('生成失败', true)
  ...
  async controlNetSetModule (e) {
    ...
    if (flag) {
      ...
      const model = findSimilar(module, modulelist)
      if (model) {
        config[e.user_id].model = model
        fs.writeFileSync(configPath, YAML.stringify(config))
  ...
  async getImage (e) {
    ...
    const result = await this.controlNet(e)
    if (result) {
      return true
    }
  }
