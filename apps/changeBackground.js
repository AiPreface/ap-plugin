...
import { parseImg, getuserName } from '../utils/utils.js'
...
if (URL) {
  API =
    'https://' +
    URL.split('/')[4] +
    '-anime-remove-background.hf.space/api/queue/'
}
...
export class example extends plugin {
  ...
  async ChangeBackground (e) {
    ...
    if (FiguretypeUser[e.user_id]) {
      e.reply(
        '当前有任务在列表中排队，请不要重复发送，处理完成后会自动发送结果，如果长时间没有结果，请等待1分钟再试'
      )
      return true
    }
    ...
    const imgdata = await canvas.loadImage(e.img[0])
    ...
    const statushash = response.data.hash
    let res = await axios.post(API + 'status/', {
      hash: statushash
    })
    ...
    const msg = e.msg.replace(/^#?更?换(背景|人物)/, '')
    const current_group_policy = await Parse.parsecfg(this.e)
    const paramdata = {
      param: {
    ...
    const resp = await Draw.get_a_pic(paramdata)
    if (resp.isnsfw) {
      // 将图片base64转换为基于QQ图床的url
      const url = await Pictools.base64_to_imgurl(resp.base64)
  ...
  async PartialRedraw (e) {
    ...
    const OriginImg = this.e.img[0]
    const BlackImg = this.e.img[1]
    const BlackImgBuffer = await axios.get(BlackImg, {
      responseType: 'arraybuffer'
    })
    const BlackImgBase64 = Buffer.from(BlackImgBuffer.data, 'binary').toString(
      'base64'
    )
    const OriginImgBuffer = await axios.get(OriginImg, {
      responseType: 'arraybuffer'
    })
    const OriginImgBase64 = Buffer.from(
      OriginImgBuffer.data,
      'binary'
    ).toString('base64')
    const MaskImg = await getBlackPixelImage(BlackImgBase64)
    if (!MaskImg) {
      return e.reply(
        '出错：重绘功能需要安装依赖：canvas\n请在yunzai根目录执行如下命令来安装依赖：\ncnpm install canvas --canvas_binary_host_mirror=https://registry.npmmirror.com/-/binary/canvas\n\n若安装依赖后仍出现此报错，您可查看控制台报错并联系开发者反馈'
      )
    }
    const msg = e.msg.replace(/^#?局部重绘/, '')
    const current_group_policy = await Parse.parsecfg(this.e)
    const paramdata = {
      param: {
        sampler: 'DPM++ 2S a Karras',
...
async function getBlackPixelImage (base64) {
  ...
  return canvas_.toDataURL().replace(/^data:image\/(png|jpg);base64,/, '')
}
...
async function invertImage (base64Image) {
  ...
  ctx.putImageData(imageData, 0, 0)
  return cvs.toDataURL().replace(/^data:image\/(png|jpg);base64,/, '')
}
