...
class Pictools extends plugin {
  ...
  async getPicInfo (param, isbs64 = false) {
    ...
    if (isbs64 || param.startsWith('http')) {
      tempPic = path.join(
        process.cwd(),
        'resources',
        `aiPainting_tempPic_${moment.now()}.png`
      )
      // 保存base64为图片
      if (isbs64) {
        fs.writeFileSync(tempPic, param, 'base64', (err) => {
          if (err) throw err
        })
      }
      // 如果是网络url则先下载
    ...
    let base64 = param
    if (!isbs64) {
      const bitMap = fs.readFileSync(tempPic)
    ...
    // 取图片长宽
    const wh = sizeOf(tempPic)
    const height = wh.height
    const width = wh.width

    // 删除下载的临时图片
    if (isbs64 || param.startsWith('http')) fs.unlinkSync(tempPic)

    // 计算图片大小
    const [b, imgsize, mb] = bs64Size(base64, true, 1)
  ...
  async base64_to_imgurl (base64) {
    Bot.pickUser(Bot.uin).sendMsg([
      segment.image(`base64://${base64}`),
      false,
      { recallMsg: 20 }
    ])
    const picinfo = await this.getPicInfo(base64, true)
    const md5 = picinfo.md5
  ...
  async text_to_qrcode (text, isurl = true) {
    ...
    if (isurl) return base64
    else return this.base64_to_imgurl(base64)
  }
  ...
  async upload_image (base64) {
    return (
      await Bot.pickFriend(Bot.uin)._preprocess(
        segment.image(`base64://${base64}`)
      )
    ).imgs[0]
  }
  ...
  async upload (content) {
    ...
    form.append(
      'ui',
      JSON.stringify(['', '', '', true, '', '', new Date().toISOString()])
    )
    form.append('optsize', '')
