...
export function getgsName(name) {
  ...
  for (let rolename of Object.values(nameArr))
    if (rolename.includes(name)) return rolename[0];
  return null;
}
...
export function chNum2Num(text, data = {}) {
  if (!("l_text" in data)) data["l_text"] = "";
  if (!("r_text" in data)) data["r_text"] = "";
  if (!("regExp" in data)) data["regExp"] = "";

  let regExp;
  if (data.regExp) regExp = new RegExp(data.regExp);
  else {
    regExp = new RegExp(
