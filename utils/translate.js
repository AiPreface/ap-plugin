...
class Translate {
  ...
  async t (text) {
    ...
    if (BAIDU.id && BAIDU.key) {
      try {
        const result = await this.BaiduTranslate(text, BAIDU.id, BAIDU.key)
        if (result) {
    ...
    if (YOUDAO.id && YOUDAO.key) {
      try {
        const result = await this.YoudaoTranslate(text, YOUDAO.id, YOUDAO.key)
        if (result) {
    ...
    // 椰奶有道
    try {
      const result = await this.yenai_youdao(text)
      if (result) {
    ...
    // Google
    try {
      const result = await this.googleTran(text)
      if (result) {
  ...
  async BaiduTranslate (msg, BDAPPID, BDAPPKEY) {
    ...
    const sign = crypto
      ...
      .digest('hex')
    const url = `${TranslateAPI}?q=${msg}&from=zh&to=en&appid=${BDAPPID}&salt=${salt}&sign=${sign}`
  ...
  async YoudaoTranslate (msg, YDAPPID, YDAPPKEY) {
    ...
    const sign = crypto
      ...
      .digest('hex')
    const url = `${TranslateAPI}?appKey=${YDAPPID}&q=${msg}&from=auto&to=en&salt=${salt}&sign=${sign}&signType=v3&curtime=${curtime}`
    const res = await fetch(url)
    const json = await res.json()
    // Log.i(json)                    /*  */
    try {
