...
async function louvre ({ img, config }, e) {
  ...
  const outputCanvas = Canvas.createCanvas()
...
const loadImage = (url, onOver) => {
  const el = new Image()
  el.src = url
}
const loadImagePromise = async (url) => {
