...
class CD {
  ...
  async checkCD (e, gpolicy) {
    ...
    if (
      ...
      const lastTime = multiPicInfo.time
      const seconds = moment(currentTime).diff(moment(lastTime), 'seconds')
      return `${gpolicy.pcd}×${multiPicInfo.count}秒个人cd（您刚刚批量绘制了${
        multiPicInfo.count
      }张图），请等待${gpolicy.pcd * multiPicInfo.count - seconds}秒后再使用`
    }
    // 个人CD--------------
    ...
    if (lastTime && !e.isMaster && gpolicy.apMaster.indexOf(e.user_id) == -1) {
      const seconds = moment(currentTime).diff(moment(lastTime), 'seconds')
      return `本群${gpolicy.gcd}秒共享cd，请等待${
        gpolicy.gcd - seconds
      }秒后再使用`
    }
    // 全局CD--------------
