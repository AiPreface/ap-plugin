...
class Draw {
  ...
  async get_a_pic (paramdata) {
    ...
    const index = paramdata.specifyAPI || config.usingAPI
    const apiobj = config.APIList[index - 1]
    ...
    const resparam = res.parameters
    // 图片大小太小，判断为全黑故障图片
    const [b, imagesize, mb] = bs64Size(base64)
    if (imagesize < 10) {
      Log.w('图片损坏')
      return {
        code: 21,
    ...
    let isnsfw = false
    if (paramdata.JH) {
      const jh = await NsfwCheck.check(base64)
      if (jh.message) {
    ...
    // 图片信息
    const picinfo = await Pictools.getPicInfo(base64, true)
  ...
  async downLoadPic (paramdata, seed, base64) {
    ...
    const currentTime = moment(new Date()).format('YYMMDD_HHmmss')
    const picname =
      `${currentTime}_${('Tags=' + param.tags + '&nTags=' + param.ntags).substring(0, 170).trim()}&seed=${seed}&user=${paramdata.user}.png`
        .replace('\n', '')
        .replace('\r', '')
        .replace('/', '／')
        .replace('\\', '＼')
...
async function constructRequestOption (param, apiobj) {
  // Log.i(param)
  const ntags = param.ntags ? param.ntags : ''
  if (!param.base64) {
