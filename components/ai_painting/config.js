...
class Config {
  ...
  initPath () {
    fs.mkdirSync(
      path.join(process.cwd(), 'resources/yuhuo/aiPainting/pictures'),
      { recursive: true }
    )
  ...
  async synccfg () {
    ...
    const defcfg = await YAML.parse(
      fs.readFileSync(
        path.join(`${Plugin_Path}/config/default_config/`, 'config.yaml'),
        'utf8'
      )
    )
  ...
  async syncplc () {
    ...
    const defpolicy = await YAML.parse(
      fs.readFileSync(
        path.join(`${Plugin_Path}/config/default_config/`, 'policy.yaml'),
        'utf8'
      )
    )
    policy = await this.syncobj(policy, defpolicy)
    policy.gp.global = await this.syncobj(
      policy.gp.global,
      defpolicy.gp.global
    )
  ...
  async syncset () {
    ...
    const defsetting = await YAML.parse(
      fs.readFileSync(
        path.join(`${Plugin_Path}/config/default_config/`, 'setting.yaml'),
        'utf8'
      )
    )
   ...
   * @return {*}
   */
  async setSetting (apcfg) {
    fs.writeFileSync(
      path.join(cfg_path, 'setting.yaml'),
      YAML.stringify(apcfg),
      'utf8'
    )
  }
   ...
   */
  mergeSetting () {
    const apcfg = YAML.parse(
      fs.readFileSync(path.join(cfg_path, 'setting.yaml'), 'utf8')
    )
    const apcfgobj = {}
    for (const key in apcfg) {
  ...
  async setcfg (apcfg) {
    fs.writeFileSync(
      path.join(cfg_path, 'config.yaml'),
      YAML.stringify(apcfg),
      'utf8'
    )
  }
  ...
  async setPolicy (policy) {
    fs.writeFileSync(
      path.join(cfg_path, 'policy.yaml'),
   ...
   * @return {*}
   *  */
  async setParse (parse) {
    fs.writeFileSync(
      path.join(cfg_path, 'parse.yaml'),
      YAML.stringify(parse),
      'utf8'
    )
  }
   ...
   * @return {*}
   */
  async setProhibitedWords (pwords) {
    fs.writeFileSync(
      path.join(cfg_path, 'prohibitedWords.yaml'),
      YAML.stringify(pwords),
      'utf8'
    )
  }
   ...
   */
  async getPresets () {
    const presets = await JSON.parse(
  ...
  async setpreSets (presets) {
    fs.writeFileSync(
      path.join(cfg_path, 'preset.json'),
      JSON.stringify(presets, null, '\t'),
      'utf8'
    )
  ...
  async getAll () {
    const apcfg = await this.getcfg()
    const plc = await this.getPolicy()
    const pwords = await this.getProhibitedWords()
