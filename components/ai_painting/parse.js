  ...
  async parsetxt (msg, is_check_preset = true, e) {
    ...
    try {
      ...
      if (loraList.length > 0) {
        const Lora = await JSON.parse(
          await redis.get('Yz:AiPainting:LoraList')
        )
        const LoraArr = []
      ...
      if (isMatchingLora) {
        e.reply(
          `匹配到Lora：${msgArr.map((lora) => lora.split(':')[1]).join(',')}`
        )
      }
    } catch (error) {
    ...
    // 中文匹配lora
    const matchLora = (await Config.getSetting()).matchLora
    if (matchLora) {
      try {
        let isMatchingLora = false
        tags = tags.replace(/，/g, ',')
        const msgArr = tags.split(',').map((tag) => tag.trim())
        const allMatchedLora = []
        const loraArr = (
          await JSON.parse(await redis.get('Yz:AiPainting:LoraList'))
        ).map((lora) => lora.name)
        for (let i = 0; i < msgArr.length; i++) {
          ...
          const matchingLora = loraArr.find((loraName) =>
            loraName.includes(msgElement)
          )
          if (matchingLora) {
